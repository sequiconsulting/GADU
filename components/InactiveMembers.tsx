...
import { UserX, Calendar, History } from 'lucide-react';
...
export const InactiveMembers: React.FC<InactiveMembersProps> = ({ members, onMemberClick, selectedYear, mode }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden animate-fadeIn min-h-[500px]">
        ...
        {inactiveList.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {inactiveList.map(member => {
              ...
              return (
                <div key={member.id} onClick={() => onMemberClick(member.id)} className="group border border-slate-200 rounded-lg p-4 hover:shadow-md hover:border-slate-300 transition-all cursor-pointer bg-slate-50/50">
                    <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-serif font-bold group-hover:bg-slate-300 transition-colors">{member.firstName[0]}{member.lastName[0]}</div>
