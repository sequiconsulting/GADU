...
export const MemberDetail: React.FC<MemberDetailProps> = ({ memberId, onBack, onSave, defaultYear }) => {
  ...
  const validateDegreePrerequisites = (branch: BranchType, degreeName: string): string | null => {
      ...
      const prerequisites: { [key: string]: { req: { branch: BranchType, name: string }[], message: string } } = {
          ...
          \'Principale dell\\\'Arco Reale\': { req: [{ branch: \'CRAFT\', name: \'Maestro Muratore\' }, { branch: \'CHAPTER\', name: \'Compagno dell\\\'Arco Reale\' }], message: \'Requisiti: Maestro Muratore (Craft) e Compagno dell\\\'Arco Reale (Chapter)\' },
      ...
      if (degreeIndex > 0) {
          ...
          // Check if there isn\'t a more specific rule already defined
